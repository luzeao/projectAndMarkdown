...
// const baseURL = 'http://localhost:3000/admin'  // 本地服务器
...
// 项目开发过程中有可能要连接多个后端(自己的  买的第三方接口(短信验证码 扫码登录 评论/留言))

// 自定义实例默认值(实例可以有多个,全局只能有一个)
// 创建实例时配置默认值
const request = axios.create({
  baseURL: 'http://localhost:3000/admin'  // 当前实例的默认请求位置
});
...
request.interceptors.request.use(function (config) {  //config 请求相关配置{methods url headers}
  ...
  var str = localStorage.getItem('Authority')  // 获取localstorage中的数据
  if (str) {  // 如果有才会携带
...
request.interceptors.response.use(function (response) {
  ...
  // 可以自己定义要返回的数据
  const res = response.data // 在响应成功的前提下获取响应数据

  const { code, message } = res

  const isSuccess = code == 200 // 响应成功,操作也成功
  const isTokenBreak = code == 10119  // 响应成功 token过期
