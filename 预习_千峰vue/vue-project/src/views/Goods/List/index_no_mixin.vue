<template>
  <div class="goodslist">
    ...
    <el-table class="list" :data="list" border style="width: 100%">
      ...
      <!-- 根据权限等级展示内容 超级管理员和普通管理员才能访问 -->
      <!-- <template v-if="isadmin">
        <el-table-column label="是否开启推荐" align="center" width="200">
          <template #default="slotScope">
            <el-switch v-model="slotScope.row.isrecommend" active-color="#13ce66" inactive-color="#ff4949"
              :active-value="1" :inactive-value="0" active-text="推荐" inactive-text="不推荐"
              @change="changeIsrecommendHandler(slotScope.row.proid, $event)" :disabled="isNormal">
            </el-switch>
          </template>
        </el-table-column>
      </template> -->
      ...
      <el-table-column label="是否开启推荐" align="center" width="200">
        <template #default="slotScope">
          <!-- 如果是普通用户禁用更改按钮 -->
          <el-switch :disabled="isNormal" v-model="slotScope.row.isrecommend" active-color="#13ce66"
      ...
      <el-table-column prop="isseckill" label="是否开启秒杀" align="center" width="250">
        <template #default="slotScope">
          <el-switch :disabled="isNormal" v-model="slotScope.row.isseckill" active-color="#13ce66"
    ...
    <el-drawer :visible.sync="isShowDrawer" direction="rtl" :with-header="false">
